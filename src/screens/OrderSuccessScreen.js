...
const PAY_LABELS = {
  cash:   { label: 'الدفع عند الاستلام', icon: '💵', color: '#0A7A3C' },
  wallet: { label: 'المحفظة',             icon: '💳', color: '#0043B0' },
  points: { label: 'النقاط',              icon: '⭐', color: '#B05200' },
};
...
export default function OrderSuccessScreen({ order, onContinue, onViewDetails }) {
  ...
  const payInfo = PAY_LABELS[order?.payMethod] || PAY_LABELS.cash;
          ...
          <View style={{
            ...
            <View style={{ flexDirection:'row', justifyContent:'space-between', alignItems:'center', paddingVertical:10, borderBottomWidth:1, borderBottomColor: theme.border }}>
              <View style={{ flexDirection:'row', alignItems:'center', gap:6 }}>
                <Text style={{ fontSize:13, fontWeight:'700', color: payInfo.color }}>{payInfo.label}</Text>
                <Text style={{ fontSize:18 }}>{payInfo.icon}</Text>
              </View>
