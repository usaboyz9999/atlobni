...
import {
  ...
  TextInput, BackHandler, Clipboard, // ✅ تم إضافة Clipboard للاستيراد
} from 'react-native';
...
export default function CouponsScreen({ onBack }) {
  ...
  // ✅ دالة جديدة للتعامل مع الضغط على الكوبون (نسخ + تطبيق)
  const handleCouponPress = (coupon) => {
    ...
    // ✅ إظهار رسالة النجاح والنسخ
    showAlert(
      '✅ تم النسخ والتطبيق', 
      `تم نسخ كود الخصم: ${coupon.code}\n(${coupon.label})\n\nتم تطبيقه في خانة التحقق أعلاه.`
    );
  };
    ...
    <View style={{ flex: 1, backgroundColor: '#EEF2F7' }}>
      ...
      <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false}>
        ...
        {COUPONS.map((coupon, i) => (
          <TouchableOpacity
            key={i}
            // ✅ تم استدعاء الدالة الجديدة بدلاً من الكود المباشر
