...
// ✅ تم حذف INITIAL_REVIEWS من هنا لأنها أصبحت في AuthContext
...
const BANNERS = [
  ...
  {
    id: 3,
    img: 'https://acs-egy.com/wp-content/uploads/2025/06/%D8%AE%D8%B7%D9%88%D8%A7%D8%AA-%D8%A7%D9%84%D8%AD%D9%81%D8%A7%D8%B8-%D8%B9%D9%84%D9%89-%D8%A3%D8%AF%D8%A7%D8%A1-%D8%A3%D9%86%D8%B8%D9%85%D8%A9-%D8%A7%D9%84%D8%B5%D9%88%D8%AA-%D9%88%D8%A7%D9%84%D8%B5%D9%88%D8%B1%D8%A9_-%D8%AF%D9%84%D9%8A%D9%84-%D8%A7%D9%84%D8%B5%D9%8A%D8%A7%D9%86%D8%A9-%D8%A7%D9%84%D8%B0%D9%83%D9%8A%D8%A9.webp',
...
export default function HomeScreen({ onSelectCategory, onGoStore, onLogout, onProfile, onGoProduct }) {
  const { t, dir } = useLang();
  // ✅ استرجاع reviews و addReview و userHasReviewed من السياق
  const { user, reviews, addReview, userHasReviewed } = useAuth();
  const showAlert = useAppAlert();
  const theme = useTheme();
  const isRTL = dir === 'rtl';
  const [filter, setFilter]       = useState('all');
  
  // ✅ تم حذف useState المحلي لـ reviews
  // نستخدم reviews القادمة من السياق مباشرة
  
  const [newRating, setNewRating] = useState(0);
  const [newText, setNewText]     = useState('');
  const [guestName, setGuestName] = useState('');
  
  // ✅ نستخدم الحالة المركزية بدلاً من المحلية
  const hasReviewed = user ? userHasReviewed : false;
  ...
  // ✅ حساب المتوسط بناءً على الآراء الواردة من السياق
  const avgRating   = (reviews.length > 0 ? (reviews.reduce((s, r) => s + r.rating, 0) / reviews.length) : 0).toFixed(1);
  ...
  // ✅ تحديث دالة submitReview لاستخدام الدالة المركزية
  function submitReview() {
    ...
    const newReviewObj = { id: Date.now(), name: displayName, rating: newRating, text: newText.trim() };
    
    // ✅ استخدام الدالة المركزية للحفظ
    ...
    Keyboard.dismiss();
    showAlert('✅ شكراً!', 'تم إرسال رأيك بنجاح');
  }
