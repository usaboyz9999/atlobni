...
export default function NewOrderModal({ visible, onClose, onSubmit }) {
  ...
  function handleSubmit() {
    if (!title.trim()) { showAlert('تنبيه', 'يرجى إدخال عنوان الطلب'); return; }
    if (selCats.length === 0) { showAlert('تنبيه', 'يرجى اختيار التخصص'); return; }
    const catLabels = selCats.map(id => CATS.find(c => c.id === id)?.label || id).join('، ');
    // جميع قطع الغيار المتاحة من القوائم المختارة
    const availableParts = partsBySub.flatMap(sg => sg.parts);
    onSubmit({
      ...
      spareParts: withParts
        ? availableParts.filter(p => selParts.includes(p.id)).map(p => p.title)
        : [],
    });
    reset(); onClose();
  }
