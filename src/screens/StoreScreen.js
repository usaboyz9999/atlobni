...
function ProductDetailScreen({ item, color, bg, onBack, onAddToCart }) {
  ...
  function handleWishlist() {
    if (!user) { showAlert('🔐 تسجيل الدخول مطلوب', 'يجب تسجيل الدخول أولاً لإضافة المنتجات للمفضلة', [
        { text: 'إلغاء', style: 'cancel' },
      ]);
      return;
    }
    toggleWishlist(item);
    if (!wished)showAlert('❤️ تمت الإضافة', `تم إضافة "${item.name}" إلى المفضلة`);
  }

  function handleAdd() {
    if (!user) {
      showAlert('🔐 تسجيل الدخول مطلوب', 'يجب تسجيل الدخول أولاً لإضافة منتجات للسلة', [
  ...
  return (
    <View style={{ flex:1, backgroundColor:'#EEF2F7' }}>
      {/* صورة + رجوع */}
      {/* ✅ التعديل: زدنا الارتفاع قليلاً لملء الفراغ مع contain */}
      <View style={{ height: 240, backgroundColor: '#fff', alignItems: 'center', justifyContent: 'center' }}>
        {item.img
...
function ProductCard({ name, desc, img, price, color, bg, code, stars, onAdd, item, onPress }) {
  ...
  function handleWishlist() {
    ...
    toggleWishlist(item);
    if (!wished)showAlert('❤️ تمت الإضافة', `تم إضافة "${name}" إلى المفضلة`);
  }
  ...
  return (
    <TouchableOpacity
      ...
      {/* صورة */}
      {/* ✅ التعديل: استخدمنا contain مع خلفية بيضاء وارتفاع مناسب */}
      <View style={{ height: 110, backgroundColor: '#fff', alignItems: 'contain', justifyContent: 'contain' }}>
        {img
...
export default function StoreScreen({ onAddToCart, initialProduct, onClearInitialProduct }) {
  ...
  return (
    <ScrollView style={[s.screen, { backgroundColor: theme.bg }]} showsVerticalScrollIndicator={false}>
      ...
      {/* ── هيرو ── */}
      <ImageBackground
        source={{ uri: 'https://cdn.salla.sa/RPNxP/bc730f52-177f-4edd-b920-9e06e5595cc9-1000x562.5-5jxLeADJ5cZekDsQ7GYehMtmXBmcISAqpKrEjK6G.jpg' }}
        style={{ height: 150 }}
      >
          ...
          >
            {subCats.map(sub => {
              ...
              return (
                ...
                >
                  <View style={{ height:70, backgroundColor: '#fff', alignItems: 'contain', justifyContent: 'contain' }}>
                    {/* ✅ التعديل: contain لضمان ظهور الصورة كاملة */}
