...
export default function HomeScreen({ onSelectCategory, onGoStore, onLogout, onProfile, onGoProduct }) {
  ...
  // ─── Animated carousel بتأثير Easing سلس ───────────────────
  const animVal    = useRef(new Animated.Value(0)).current;
  const currentIdx = useRef(0);
  const isPaused   = useRef(false);

  const goToSlide = (idx, duration = 700) => {
  ...
  // translateX مقلوب لأن ScrollView يتحرك يسار ← يمين
  const translateX = animVal.interpolate({
    inputRange:  [0, BANNER_W, BANNER_W * 2],
    outputRange: [0, -BANNER_W, -BANNER_W * 2],
  });
  ...
  function submitReview() {
    ...
    Keyboard.dismiss();
    Alert.alert('شكراً!', 'تم إرسال رأيك بنجاح ✅');
  }
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      ...
      >
        {/* ── البانر ── */}
        <ImageBackground source={{ uri:'https://images.unsplash.com/photo-1581092580497-e0d23cbdf1dc?w=700&q=80' }} style={s.heroBanner}>
          ...
          <View style={[s.heroContent,{alignItems:isRTL?'flex-end':'flex-start'}]}>
            <View style={s.logoRow}>
              ...
              <View>
                <Text style={[s.appNameSm,{textAlign:isRTL?'right':'left',fontSize:18,fontWeight:'900',letterSpacing:1}]}>{t('appName')}</Text>
