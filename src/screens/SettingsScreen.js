...
function EditProfileScreen({ onBack }) {
  ...
  function save() {
    if (!name.trim()) { Alert.alert('تنبيه', 'الاسم لا يمكن أن يكون فارغاً'); return; }
    updateUser({ name: name.trim(), phone: phone.trim(), avatar: name.trim()[0]?.toUpperCase() });
    Alert.alert('✅ تم الحفظ', 'تم تحديث بياناتك بنجاح', [{ text: 'حسناً', onPress: onBack }]);
  }
...
function ChangePasswordScreen({ onBack }) {
  ...
  function save() {
    if (!current)            { Alert.alert('تنبيه', 'أدخل كلمة المرور الحالية'); return; }
    if (current !== user.password) { Alert.alert('خطأ', 'كلمة المرور الحالية غير صحيحة'); return; }
    if (newPass.length < 6)  { Alert.alert('تنبيه', 'كلمة المرور الجديدة يجب أن تكون 6 أحرف على الأقل'); return; }
    if (newPass !== confirm)  { Alert.alert('تنبيه', 'كلمتا المرور الجديدتان غير متطابقتان'); return; }
    updateUser({ password: newPass });
    Alert.alert('✅ تم التغيير', 'تم تغيير كلمة المرور بنجاح', [{ text: 'حسناً', onPress: onBack }]);
  }
    ...
    <View style={{ flex:1, backgroundColor:'#EEF2F7' }}>
      ...
      <ScrollView style={{ flex:1 }}>
        <View style={{ padding:14 }}>
          ...
          <TouchableOpacity onPress={() => Alert.alert('طلب حذف الحساب', 'سيتم إرسال طلب حذف حسابك وبياناتك إلى فريق الدعم. سيتم معالجة الطلب خلال 7 أيام عمل.')} style={{
...
export default function SettingsScreen({ onBack, onGoOrders }) {
  ...
  function handleRate() {
    Alert.alert('⭐ قيّم التطبيق', 'هل أنت سعيد بتجربتك مع اطلبني؟', [
      { text: 'لاحقاً', style: 'cancel' },
      { text: '⭐⭐⭐⭐⭐ ممتاز!', onPress: () => Alert.alert('شكراً جزيلاً! 💙', 'رأيك يساعدنا على تقديم الأفضل دائماً') },
    ]);
  }
      ...
      <ScrollView style={{ flex:1 }} showsVerticalScrollIndicator={false}>
        <View style={{ paddingHorizontal:14 }}>
          ...
          <SettingRow icon="💬" label="تواصل معنا" desc="support@atlobni.com"
            iconBg="#E6F0FF" theme={theme}
            onPress={() => Alert.alert('تواصل معنا 💬', 'البريد: support@atlobni.com\nالهاتف: 920-XXX-XXX\nساعات الدعم: 9ص - 6م')} />
