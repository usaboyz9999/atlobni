...
export default function AuthScreen({ onSuccess }) {
  const { login, register, applyReferral, validateReferralCode } = useAuth();
  ...
  function handleRegister() {
    ...
    // التحقق من كود الإحالة إذا أُدخل
    if (referralCode.trim()) {
      const check = validateReferralCode(referralCode.trim(), 'device_' + email);
      if (!check.valid) { e.referral = check.error; }
    }
    ...
    // تطبيق كود الإحالة بعد التسجيل الناجح
    if (referralCode.trim()) {
      applyReferral(referralCode.trim(), 'device_' + email);
    }
    setShowSuccess(true);
  }
              ...
              <View style={{ marginBottom:12 }}>
                ...
                <TextInput
                  ...
                  style={{
                    ...
                    fontSize:14, color:'#0A2463', textAlign:'right',
                    borderWidth:1.5,
                    borderColor: errors.referral ? '#E53935' : referralCode ? '#0A7A3C' : '#DDE4EF',
                    fontWeight:'700', letterSpacing:1,
                  }}
                  underlineColorAndroid="transparent"
                />
                {errors.referral ? (
                  <Text style={{ color:'#E53935', fontSize:10, textAlign:'right', marginTop:4 }}>⚠️ {errors.referral}</Text>
                ) : referralCode ? (
                  <Text style={{ color:'#0A7A3C', fontSize:10, textAlign:'right', marginTop:4 }}>✓ سيتم التحقق من الكود عند التسجيل</Text>
                ) : (
                  <Text style={{ color:'#6B7C93', fontSize:10, textAlign:'right', marginTop:4 }}>أدخل كود من صديق للحصول على مكافأة</Text>
                )}
              </View>
